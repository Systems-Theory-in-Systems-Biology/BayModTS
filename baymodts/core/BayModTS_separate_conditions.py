...
model_name = 'baymodts' #'weight_model' # 'baymodts' #

Conditions = ['T', 'F'] #['young', 'middle', 'old'] #['LM', 'LL', 'RI', 'RS', 'RM', 'SC', 'IC']#  # ['major', 'minor']#  #

Path = Path(__file__).parent.parent.parent / 'Project_Clinics' / 'liver_resection' / 'Polypharmacy_start_postop'#'multimorbid_125days' #
...
class Measurement_data:
    ...
    def create_measurement_df(self):
        ...
        data = pd.read_csv(
            os.path.join(self.Path, f'{model_name}_measurement_table.tsv'), #_norm
            sep='\t')
...
class PEtab_problem:
    ...
    def __init__(self, condition: str, Path):
        ...
        petab_yaml_dict['problems'][0]['condition_files'] = \
            [f'{model_name}_experimental_conditions.tsv']
    ...
    def plot_ensemble(self):
        ...
        plt.plot(Current_data.cond_data['time'].to_numpy(),
                ...
                marker='+', color='k', linestyle='', markersize=15)
