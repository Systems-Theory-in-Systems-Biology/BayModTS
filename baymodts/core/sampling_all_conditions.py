...
def _plot_trajectories_by_condition(
    ...
    alpha:
	  can alter the alpha of the fills of the credibilit intervals
    median_color:
        ...
        for output_index, output_id in enumerate(output_ids):
            ...
            ax.plot(
                *_get_statistic_data(summary, MEDIAN, condition_id, output_id),
                color = median_color, label=labels[condition_id]
            )
...
def sampling_prediction_trajectories(
    ...
    color_add:
	  Influence the color of each plot and therefore make different colors
	  for the same plot call possible. (Custom)
    alpha_credibility:
	  Can adjust the alpha of the fill of the credibility intervalls - currently only for group by CONDITION
    median_color:
	  Can specify the color of the median line - currently only for group by CONDITION
    Returns
    -------
    axes:
        The plot axes.
