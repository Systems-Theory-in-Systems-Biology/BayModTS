...
class Measurement_data:
    ...
    def create_measurement_df(self, path):
        ...
        observable_array = [f'{self.df_name[:3]}_plasma_obs' for i in range(self.number_of_replicates * len(self.t))]
    ...
    def create_parameters_table(self):
        ...
        par_data = {
            'parameterId': [f'Mr_{self.df_name[:3]}', 'conc_conversion',  'kabs', 'CL', 'Q', 'sd1_y_obs'],
            'parameterScale': ['lin' for parameter in range(6)],
            'lowerBound': [1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3],
            'upperBound': [1e+2, 1e+2, 1e+2, 1e+2, 1e+2, 1e+3],
            'estimate': [0, 0, 1, 1, 1, 1],
            'nominalValue': [molecular_weights[self.df_name], 1000, 0.01, 0.01, 0.1, 1]
        }
        self.parameter_df = pd.DataFrame(par_data)
        # Save parameter table
        self.parameter_df.to_csv(os.path.join(p, f'parameter_{self.df_name}_pk.tsv'), sep='\t', index=False)

    def create_observables_table(self):
        """Create PEtab observables file"""

        obs_data = {
            'observableId': [f'{self.df_name[:3]}_plasma_obs'],
            'observableName': [f'{self.df_name} plasma (ng/ml)'],
            'observableFormula': [f'{self.df_name[:3]}_plasma'],
            'noiseFormula': [f'noiseParameter1_{self.df_name[:3]}_plasma_obs'],
            'noiseDistribution': ['normal']
        }

        self.observable_df = pd.DataFrame(obs_data)
        # Save parameter table
        self.observable_df.to_csv(os.path.join(p, f'observables_{self.df_name}_pk.tsv'), sep='\t', index=False)
...
class PEtab_problem:
    ...
    def MCMC_sampling(self):
        ...
        PEtab_yaml_name = f'{self.substance}_pk_{self.condition}.yaml'
        ...
        store.save_to_hdf5.write_result(self.result,
            os.path.join(p, f'results_pk/{self.substance}_{self.condition}_MCMC_chain.hdf5'),
            overwrite=True)
    ...
    def MCMC_visualization(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(p, f'results_pk/{self.substance}_{self.condition}_traces.png'))

        # Credibility interval plots (Boxplot alike depiction)
        alpha = [ci]
        ax = visualize.sampling_parameter_cis(self.result, alpha=alpha)
        plt.tight_layout()
        plt.savefig(os.path.join(p, f'results_pk/{self.substance}_{self.condition}_credibility_intervals.png'))

        ax = visualize.sampling_scatter(self.result, stepsize=100,
                                        show_bounds=False)
        plt.tight_layout()
        plt.savefig(os.path.join(p, f'results_pk/{self.substance}_{self.condition}_sampling_scatter.png'))

        ax = visualize.sampling_1d_marginals(self.result, plot_type='both') # i_chain: int # default is first chain
        plt.tight_layout()
        plt.savefig(os.path.join(p, f'results_pk/{self.substance}_{self.condition}_1d_marginals.png'))
        ...
        # save ess
        with open(os.path.join(p, f'results_pk/ESS.txt'), 'a') as ess_file:
    ...
    def Ensemble_creation(self):
        ...
        try:
            os.remove(os.path.join(p, f'results_pk/{self.substance}_{self.condition}_ensemble_prediction.hdf5'))
        except OSError:
            pass
        write_ensemble_prediction_to_h5(
           self.ensemble_prediction,
           os.path.join(p, f'results_pk/{self.substance}_{self.condition}_ensemble_prediction.hdf5'))
    ...
    def plot_ensemble(self):
        ...
        ax = sampling_prediction_trajectories(
            ...
            output_ids=[f'{self.substance[:3]}_plasma_obs'],   # must be observables - None, for all outputs
...
def all_conditions_plot(UQ_control_object, UQ_2Wks_object, UQ_4Wks_object):
    ...
    for condition_object in [UQ_control_object, UQ_2Wks_object, UQ_4Wks_object]:
        sampling_prediction_trajectories(
            ...
            groupby=CONDITION,
            condition_ids=None,  # None, for all conditions
            output_ids=[f'{UQ_control_object.substance[:3]}_plasma_obs'],  # must be observables - None, for all outputs
...
if __name__ == '__main__':
    ...
    for df_name in Substances:
        ...
        df_name_control.create_observables_table()
        ...
        df_name_2Wks.create_observables_table()
        ...
        df_name_4Wks.create_observables_table()
